...
class Interaction(object):
    ...
    def to_user_item_ratings(self):
        if self._data.empty:
            warnings.warn("self._data is empty.")
            return None
        # users_np = self._data[_USER].to_numpy(copy=True, dtype=np.int32)
        # items_np = self._data[_ITEM].to_numpy(copy=True, dtype=np.int32)
        uir_triplets = self._data[[_USER, _ITEM, _RATING]].to_numpy(copy=True, dtype=np.int32)
        return uir_triplets
    ...
    def to_truncated_seq_dict(self, max_len, pad_value=0, padding='post', truncating='post'):
        ...
        """
        user_seq_dict = self.to_user_dict()
        if max_len is None:
...
class Dataset(object):
    ...
    def _load_data(self, data_dir, sep, columns):
        ...
        self.all_data = Interaction(all_data, num_users=self.num_users, num_items=self.num_items)
        ...
        if columns in [['user', 'item', 'rating', 'time']]:
            self.time_matrix = sp.csr_matrix((_train_data[_TIME], (_train_data[_USER], _train_data[_ITEM])),
                                          shape=(self.num_users, self.num_items))
            self.time_test_matrix = sp.csr_matrix((_test_data[_TIME], (_test_data[_USER], _test_data[_ITEM])),
                                          shape=(self.num_users, self.num_items))

        user_map = pd.read_csv(user2id_file, sep=sep, header=None, names=["user", "id"])
        item_map = pd.read_csv(item2id_file, sep=sep, header=None, names=["item", "id"])
        ...
        self.train_matrix = sp.csr_matrix((_train_data[_RATING], (_train_data[_USER], _train_data[_ITEM])),
                                       shape=(self.num_users, self.num_items))
        self.test_matrix = sp.csr_matrix((_test_data[_RATING], (_test_data[_USER], _test_data[_ITEM])),
                                      shape=(self.num_users, self.num_items))
